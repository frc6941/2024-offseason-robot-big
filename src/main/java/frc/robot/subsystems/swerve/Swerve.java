...
public class Swerve implements Updatable, Subsystem {
    ...
    private void updateModules(HolonomicDriveSignal driveSignal, double dt) {
        ...
        for (SwerveModuleBase mod : swerveMods) {
            System.out.println(setpoint.mModuleStates[mod.getModuleNumber()]);
            mod.setDesiredState(setpoint.mModuleStates[mod.getModuleNumber()], driveSignal.isOpenLoop());
        }
    }
    ...
    private void setModuleStatesBrake() {
        for (SwerveModuleBase mod : swerveMods) {
            ...
            mod.setDesiredState(new SwerveModuleState(0.0, angle.plus(Rotation2d.fromDegrees(180.0))), false);
        }
    }
