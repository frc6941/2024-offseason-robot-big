...
public class IntakerIOTalonFX implements IntakerIO {
	private final TalonFX intakeTalon = new TalonFX(INTAKE_MOTOR_ID, Constants.RobotConstants.CAN_BUS_NAME);
	...
	public void updateInputs(IntakerIOInputs inputs) {
		...
		inputs.intakeVelocity = RadiansPerSecond.of(Units.rotationsToRadians(intakeVelocity.getValueAsDouble()));
		inputs.intakePosition = Radians.of(Units.rotationsToRadians(intakePosition.getValueAsDouble()));
		inputs.intakeAppliedVoltage = Volts.of(intakeAppliedVoltage.getValueAsDouble());
		inputs.intakeSupplyCurrent = Amps.of(intakeSupplyCurrent.getValueAsDouble());
	}
